Cypress.viewports.forEach(({ name, width, height }) =>
  describe(`${name} - Character Creation End-to-End`, () => {
    ...
    it('should complete the full character sheet happy path workflow', () => {
      ...
      cy.getByTestId('race-card-').should('have.length', name === 'Mobile' ? 1 : 3);
      ...
      cy.getByTestId('race-choices-proficiency').within(() => {
        ...
        cy.get('label:visible')
          .should('have.length', 1)
          .should('contain.text', "Brewer's Supplies");
      });
      ...
      cy.getByTestId('class-carousel').should('be.visible');
      cy.get('button:contains("Next"):visible').should('be.disabled');
      cy.getByTestId('class-card-').should('have.length', name === 'Mobile' ? 1 : 3);
